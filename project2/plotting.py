...
def plot_rnn_vs_true(targets, predictions, params, select_time, pred_time, mse):
	...
	ax.set_title('Model ' + str(select_time) + 'True vs RNN with input length '+ str(pred_time) + ', lr = ' + str(params[0]) + " ,batch = " + str(params[1])
		 + ", alpha = " + str(params[4])+ ', MSE=' +str(mse))
	fig5.set_size_inches(18.5, 10.5, forward=True)
	plt.tight_layout()
	plt.subplots_adjust(top=0.85)
	fig5.savefig('last epoch 40: Model ' + str(select_time) + 'True vs RNN with input length '+ str(pred_time)+'.png')
...
def plot_baseline_vs_true_2(targets, baseline_predictions):
	fig = 7
	#fig2, axarr = plt.subplots(2, sharex=True)
	plt.scatter(targets, baseline_predictions, edgecolors=(0, 0, 0))
	plt.plot([min(targets), max(targets)], [min(targets), max(targets)], 'k--', lw=4)
