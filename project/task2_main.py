...
if __name__ == "__main__":
	...
	# prepare data for cv
	rnn_input = prepare_rnn_input(data_scaled, targets_selected, select_time).reshape(-1, select_time -1, 1 + data.shape[1])
	rnn_targets = prepare_rnn_targets(targets_selected, select_time).reshape(-1,select_time -1)
	...
	loss = best_model.fit(train_rnn_input.reshape(-1, select_time -1, 1 + data.shape[1]), 
		train_rnn_targets.reshape(-1,select_time -1),
	...
	predictions = best_model.predict(rnn_input.reshape(-1, select_time -1, 1 + data.shape[1]))
	...
	for t in time:
		...
		for s in range(t , 41):
			...
			rnn_input = prepare_rnn_input(data_scaled, reshaped_transposed_predictions, select_time).reshape(-1, 1, 1 + data.shape[1])
